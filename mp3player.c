  ...
  short *_buf = (short *)_buf2;
	...
	while (samplesOut < numSamples) {
		...
		if (MAD_RECOVERABLE(Stream.error)) {
			...
			//**************commented by Devun_06************//printf("recoverable frame level error (%s)\n", MadErrorString(&Stream));
			sceDisplayWaitVblankStart();
		    }
		    return;	//continue;
		    ...
		    //**************commented by Devun_06************//printf("unrecoverable frame level error (%s).\n", MadErrorString(&Stream));
		    sceDisplayWaitVblankStart();
		    Status = 1;
		    MP3_Stop();	//break;
		}
...
void MP3_FreeTune()
{
	...
	//**************commented by Devun_06************//printf("%lu frames decoded (%s).\n", FrameCount, Buffer);
	sceDisplayWaitVblankStart();
