...
{
char*ImageType;ImageType=strrchr(filename,'.');
//*PNG
if(stricmp(ImageType,".png")==0)
 {
	png_structp png_ptr;
	png_infop info_ptr;
...
//*JPG
if(stricmp(ImageType,".jpg")==0||stricmp(ImageType,".jpeg")==0)
{
	int x=0;
...
void blitAlphaImageToScreen(int sx, int sy, int width, int height, Image* source, int dx, int dy/*, int resize*/)
{
	if (!initialized) return;
	...
	while (j < width) {
		...
		int sliceWidth = 64;//int sliceWidth = 64*(resize);//this line edited by : Devun_06 ;)
...
void printTextScreen(int x, int y, const char* text, u32 color)
{
	...
	for (c = 0; c < strlen(text); c++) {
        if(/*1*/x<=((8/4)*-1)/*2*/){x+=8;continue;}//<-Not here originally...
        else//<-Not here originally...
		if (/*x < 0  ||*/ x + (8/8) > SCREEN_WIDTH || y < 0 || y + 8 > SCREEN_HEIGHT) break;
		char ch = text[c];
